...
def mostrar_titulo(seccion: int, intentos: int = 0):
    ...
    try:
        ...
        raise ValueError(f"*ERROR* - El siguiente error es: {e}")
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if numero - numero_oculto > 15 or numero - numero_oculto < -15:
        return 0
    
    if numero - numero_oculto > 5 or numero - numero_oculto < -5:
        return 1
    
    if numero - numero_oculto > 1 or numero - numero_oculto < -1:
        return 2
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    total_intentos = configurar_intentos()
    pedir_numero_usuario("¿Qué número es? ", minimo, maximo)
    while numero_oculto != pedir_numero_usuario():
...
def configurar_rangos_numeros() -> tuple:
    ...
    try:
        ...
        if maximo - minimo < 0:
            ...
            raise ValueError(f"*ERROR* - El error es el siguiente: {e}")
...
def configurar_intentos(rango_numero_oculto) -> int:
    ...
    minimo = 1
    maximo = 1
    rango_numero_oculto = configurar_rangos_numeros(minimo, maximo)
    ...
    try:
        ...
        if intentos < rango_numero_oculto:
            raise ValueError("*ERROR* El número de intentos no puede ser superior al 10/100 del rango del número oculto!")
