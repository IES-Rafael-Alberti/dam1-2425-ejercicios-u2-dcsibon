...
def preguntar_total(desc: str, min: int)-> int:
    ...
    while not total_correcto:
        try:

            total = pedir_numero(f"Total de {desc} a jugar ({min}-{max})?>> ",min,max)

            if total < min or total > max:
                raise ValueError("Error, el número tiene que estar entre el mínimo y el máximo.")
...
def solicitar_numeros(desc: str, total: int, min: int, max: int)->set:
    ...
    for i in range(1, total+ 1):
        numero = pedir_numero((f"Dame el número #{i}#>> "), min, max)
...
def main():
    ...
    estrellas_jugadas = solicitar_numeros("el número", total_estrellas, CONFIG["estrellas"]["min"], CONFIG["estrellas"]["max"])
