...
def multiplicar(num1:float, num2: float) -> int:
    ...
    if es_resultado_negativo(num1, num2) == True:
        ...
        if str(num1).startswith("-") == True:

            num1 = num1[1:]
        
        if str(num2).startswith("-") == True:
...
def dividir(num1: float, num2: float) -> int:
    ...
    if es_resultado_negativo(num1, num2) == True:
        ...
        while num1 >= num2:  #   Mientras sea divisible (Dividendo mayor que divisor) se hará la operación.
            num1 -= num2    #   Se irá restando el divisor al dividendo, ya que eso nos diría que es divisible.
            resultado += 1  #   Cada vez que el divisor quepa es un número más al cociente.
...
def realizar_calculo(decimales):
    """
...
def main():
    ...
    while not desea_salir:
        try:
            ...
            if entrada == "":
                if pedir_entrada("¿Desea salir de la calculadora? (s/n): ") == "s":
                    desea_salir = True
                ...
                resultado = realizar_calculo(decimales)
