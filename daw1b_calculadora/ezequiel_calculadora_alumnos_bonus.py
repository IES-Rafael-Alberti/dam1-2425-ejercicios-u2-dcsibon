...
def sumar(num1: float, num2: float) -> float:
    ...
    Args:
        n1 (float): Número 1 a sumar
        n2 (float): Número 2 a ser sumado

    Returns:
        float: La suma del Núemro 1 más el Número 2 (n1+n2)
    """
...
def restar(num1: float, num2: float) -> float:
    ...
    Args:
        n1 (float): Número 1 para restar
        n2 (float): Número 2 a ser restado

    Returns:
        float: La resta del Número 1 menos el Número 2 (n1-n2)
    """
...
def es_resultado_negativo(num1: float, num2: float) -> bool:
    ...
    num1_es_negativo = num1 < 0
    num2_es_negativo = num2 < 0

    if num1_es_negativo != num2_es_negativo:
        return True
    else:
        return False
...
def dividir(num1: float, num2: float) -> int:
    ...
    es_negativo = es_resultado_negativo(num1, num2) # Llamamos a la función para comprobar si es negativo o no
    ...
    if num2 > num1:
        ...
        while resultado_temp >= 0:
            resultado_temp = resultado_temp - num2
            counter += 1

    resultado = counter - 1

    # En caso que alguno de los dos números sea negativo, cambiaremos el resultado a negativo, pues un número positivo entre un número negativo, debe retonar un número negativo.
    if es_negativo:
        resultado = -resultado
...
def potencia(num1: float, num2: float) -> int:
    """
    Realiza la potencia entre el num 1 y num2 (num1**num2)
    ...
    try:
        if num2 == 0:
            ...
            num1_abs = abs(round(num1))
            num2_abs = abs(round(num2))
            resultado = 0
            for _ in range(num2_abs):
                multiplicacion = multiplicar(num1_abs, num1_abs)
                resultado = resultado + multiplicacion

            if num2_abs % 2 != 0 and num1 < 0:
                resultado = -resultado
...
def calcular_operacion(num1: float, num2: float, operador: str) -> float:
    ...
    if operador == "+":
        ...
        # El Raises de ZeroDivisionError ya lo trae la "función dividir()", pues ahí en la documentación pide que se haga un Raise tambien.
        # Asi que no he hecho otro Raise en esta funciíon "calcular_operacion()", pues al ya haber hecho un Raise lo capturará sin problemas "realizar_calculo()"
...
def obtener_operaciones() -> str:
    ...
    """
    operaciones_disponibles = """Operaciones disponibles:
  ce => Reiniciar resultado a 0
  decimales <n> => Establecer decimales en resultado
  cadena vacía + <ENTER> => Pregunta si desea salir
  calculo => Iniciar cálculo secuencial
...
def realizar_calculo(decimales: int, resultado_almacenado: float) -> float:
    ...
    try:
        ...
        while realizando_calculos:
            ...
            if resultado == None:
                entrada = pedir_entrada(f"\t (Cálculo = {0:.{decimales}f}) >> ")
            else:
                entrada = pedir_entrada(f"\t (Cálculo = {resultado:.{decimales}f}) >> ")
...
def main():
    ...
    print("Menú")
    print("1. Realizar un cálculo secuencial.")
    print("2. Lista de operaciones disponibles para el cálculo.")
    print("3. Reiniciar resultado (CE).")
    print("4. Configurar número de decimales.")
    print("5. Salir.\n")
    ...
    while not desea_salir:
        ...
        if resultado == None:
            entrada = pedir_entrada(f"Operación (RES => {0:.{decimales}f}) >> ")
        else:
            entrada = pedir_entrada(f"Operación (RES => {resultado:.{decimales}f}) >> ")
 
        if entrada == "":
            salir_opcion = pedir_entrada("¿Desea salir de la calculadora? (s/n) ")
            if salir_opcion == "s" or salir_opcion == "si":
                desea_salir = True

        elif entrada == "lista":
            lista_operaciones = obtener_operaciones()
            ...
            try:
                decimales = int(str(entrada.split(' ')[1]))
