...
def multiplicar(num1, num2):
    ...
    for _ in range(abs(num2)):
        resultado += num1
    return resultado if num2 >= 0 else -resultado

    #Aquí se puede observar como sin usar *, hemos conseguido realizar una multiplicación. Redondeamos primero num1 y num2 para, por ejemplo
...
def dividir(num1, num2):
    ...
    while temp >= abs(num2):
        temp -= abs(num2)
        resultado += 1
    return resultado if es_resultado_negativo(num1, num2) else -resultado
...
def realizar_calculo(decimales, resultado_almacenado):
    ...
    while realizando_calculos:
        entrada = pedir_entrada("(Cálculo = {}) >> ".format(resultado if resultado else 0))
        ...
        else:   # TODO: este código funciona cuando solucionéis que reconozca las variables resultado_almacenado y decimales.
...
def main():
    ...
    while not desea_salir:
        entrada = pedir_entrada("Operación (RES => {:.{}f}) >> ".format(resultado, decimales))
        
        if entrada == "":
            salida = pedir_entrada("¿Desea salir de la calculadora? (s/n) ")
            if salida == "s":
                limpiar_pantalla()
                print("\n\nBye, bye...\n\n")
                desea_salir = True
