...
def sumar(operador, numero):

    num1 = round(num1)
    num2 = round(num2)

    es_resultado_negativo = (num1 < 0) != (num2 < 0)
...
def realizar_calculo():
    ...
    print("Comienza tu secuencia de cálculos. Escribe 'cancelar' para cancelar en cualquier momento.")
    
    while True:
        if operando:
        
            entrada = input(f"Introduce un número (o 'cancelar' para salir, 'resultado' para usar el último resultado): ").strip()
            
            if entrada.lower() == 'cancelar':
                print("Cálculo cancelado.")
                return None 
            
            if entrada.lower() == 'resultado':
            ...
            if resultado == 0.0:
                ...
                if operador == "+":
                    ...
                    print("Operador no válido. Debes usar uno de los siguientes: +, -, *, /.")
                    continue  

        
        continuar = input(f"Resultado actual: {resultado}. ¿Quieres realizar otra operación? (s/n): ").strip().lower()
...
def main(comando, obtener_operadores):
    ...
    print("        <ENTER> => Volver actualizando resultado de la calculadora")
    print("    limpiar => Limpiar la pantalla de la consola")
