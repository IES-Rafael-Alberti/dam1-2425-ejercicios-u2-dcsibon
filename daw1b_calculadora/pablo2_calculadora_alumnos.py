...
def limpiar_pantalla():
    ...
    try:
        ...
        mostrar_error(f"Problemas al intentar limpiar la pantalla")
...
def pausa():
    ...
    Args: 
    tecla_enter (bool): True si quieres aplicar el enter.
    """
...
def mostrar_error(indice_error: int, msj_error = None):
    ...
    try:
        ...
        if msj_error is not None:
            print(f"\n*ERROR* {mensaje.format(error=str(msj_error))}\n")
        else:
            print(f"\n*ERROR* {mensaje}\n")
    except IndexError:
        print("\n*ERROR* Índice de error fuera de rango\n")
...
def potencia(base: float, exponente: float) -> float:
    """
    Realiza la potencia de dos números usando la funcion de multiplicar.
    
    Args:
        base (float): base.
        exponente (int): Potencia.
...
def realizar_calculo(decimales: int, resultado_almacenado: float) -> float:
    ...
    while realizando_calculos:
        ...
        if entrada == "cancelar":
            ...
            print("Calculo cancelado, volvemos sin realizar cambios!")
            ...
            #    - ZeroDivisionwError que debe mostrar el error que está en la posición 5 de MENSAJES_ERROR.
            ...
            try:
                ...
                elif resultado is 0 and operador is None:
                    resultado = numero
...
def main():
    ...
    while not desea_salir:
        ...
        if entrada == "":
            entrada = pedir_entrada("¿Desea salir de la calculadora?")
            print("\n\nBye, bye...\n\n")
            desea_salir = True
            ...
            digitos = entrada.split()
            if len(digitos) != 2 or not digitos[1].isdigit():
                mostrar_error(2)
            else:
