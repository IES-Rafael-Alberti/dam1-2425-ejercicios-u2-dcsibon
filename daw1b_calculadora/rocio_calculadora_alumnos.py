...
def mostrar_error(indice_error = None, msj_error = None):
    """
    Muestra un mensaje de error en la consola. En caso de que no se especifique
    el error muestra el mensaje 'error no definido'.
    ...
    Raises:
        NameError: cuando el indice_error no está dentro del rango 0-6.
    """
    # TODO: 
    ...
    try:
        if indice_error == None:
            print("\n*ERROR* Mensaje de error no definido.\n")

        elif indice_error < 0 or indice_error > 6:
            raise NameError("No se ha indicado el índice del error")
...
def sumar(num1: float, num2: float) -> int:
    """
    Esta función realiza la suma entre dos números.

    Args:
        num1 (float): número introducido.
        num2 (float): número introducido.

    Returns:
        res (int): número producto de la suma.
    """
...
def restar(num1: float, num2: float) -> int:
    """
    Esta función realiza la resta entre dos números.
    ...
    Returns:
        res (int): número producto de la resta.
    """
...
def es_resultado_negativo(num1: float, num2: float) -> bool:
    ...
    Returns:
        negativo (bool): valor que nos dice si es True que el resultado debe ser negativo o False
        si debe ser positivo.
    """
...
def multiplicar(num1: float, num2: float) -> int:
    ...
    Llama a la función es_resultado_negativo() para comprobar si el resutlado de la 
    operación debe ser positivo (False) o negativo (True).
    ...
    Returns:
        res (int): Resultado de la multiplicación.
    ...
    for i in range(0, round(num2)+1, 1):
        res += round(num1)
...
def dividir(num1: float, num2: float) -> int:
    ...
    dividendo = round(abs(num1))
    divisor = round(abs(num2))
    negativo = es_resultado_negativo(num1, num2)
    res = 0
...
def realizar_calculo(decimales: int, resultado_almacenado: float) -> float:
    ...
    Args:
        ...
        resultado (float): Valor almacenado en la calculadora.
...
def main():
    ...
    while not desea_salir:
        ...
        entrada = pedir_entrada(f"Operación (RES => {resultado_almacenado}) >> ")

        if entrada == "":
            salir = pedir_entrada("¿Desea salir de la calculadora? (s/n) ").upper()
            if salir == 'S' or salir == 'SI' or salir == 'SÍ':
                desea_salir = True
            elif salir == 'N' or salir == 'NO':
                desea_salir = False
            else:
                mostrar_error(6, "Debe ser una respuesta de sí o no")
