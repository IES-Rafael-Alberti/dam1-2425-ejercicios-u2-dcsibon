...
def redondear(num: float) -> int:
    """ Redondea el número
    Args:
        num (float): Número decimal
    Returns:
        int: El número redondeado
    """
    num = (str(num)).split(".")
    parte_entera = int(num[0])
    parte_decimal = float("0." + str(num[1]))
    if round(parte_decimal,2) >= 0.50:
...
def multiplicar(num1: float, num2: float) -> int:
    ...
    resultado = 0
    if num1 > num2:
        for i in range (1,num1+1): # suma el número el número de veces que es num2
            resultado += num2
    else:
        for i in range (1,num2+1): # suma el número el número de veces que es num2
            resultado += num1
    
    if negativo: # si el resultado era negativo le añade el menos
...
def dividir(num1: float, num2: float) -> int:
    ...
    if es_resultado_negativo(num1, num2):
        negativo = True
        num1 = int((str(num1)).replace("-",""))
        num2 = int((str(num2)).replace("-",""))
    contador = 0 
    if num2 == 0:
        raise ZeroDivisionError
    for i in range (num2,num1+1,num2): #recorre desde num2 a num1 (de num2 en num2) y cada vez suma uno al contador
        contador +=1
    if negativo:
        contador = int("-" + str(contador) )
    return contador
...
def potencia(num1: float, num2: float) -> int: # llamar a multiplicar
    """
    Realiza la potencia

    Args:
        num1 (float): primer numero
        num1 (float): segundo numero
    Returns:
    ...
    if num2 == 0:
        ...
        contador = num2 - 1
        while contador > 0:
            resultado = multiplicar(abs(num1), resultado)
            contador -= 1
...
def realizar_calculo(decimales: int, resultado_almacenado: float) -> float:
    ...
    num1 = float(pedir_entrada(f"\t (Cálculo = {resultado}) número>> ")); resultado = num1
    operador = pedir_entrada(f"\t (Cálculo = {resultado}) operación>> ")
    num2 = float(pedir_entrada(f"\t (Cálculo = {resultado}) número>> "))
    resultado = calcular_operacion(resultado, num2, operador)

    while realizando_calculos:
        resultado_almacenado = resultado
        entrada = pedir_entrada(f"\t (Cálculo = {resultado}) operación>> ")
        ...
        if entrada == "cancelar":
            ...
            operador = entrada
            entrada = float(pedir_entrada(f"\t (Cálculo = {resultado}) número>> "))
            resultado = calcular_operacion(resultado, entrada, operador)
