...
def pausa(tiempo = 1, tecla_enter = False, limpiar = False):
    """
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if numero > numero_oculto:
        distancia_entre_num = numero - numero_oculto
    else:
        distancia_entre_num = numero_oculto - numero
    if distancia_entre_num > caliente:
        return 0
    elif distancia_entre_num <= caliente:
        return 1
    elif distancia_entre_num < caliente//2:
        return 2
...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    if numero > numero_oculto:
        if evaluar_diferencia(numero, numero_oculto, frio, caliente) == 0: print("* FRÍO, FRÍO, el número oculto es MENOR... ¡te quedan N intentos!\n")
        elif evaluar_diferencia(numero, numero_oculto, frio, caliente) == 1: print("* CALIENTE, CALIENTE, el número oculto es MENOR... ¡te quedan N intentos!\n")
        elif evaluar_diferencia(numero, numero_oculto, frio, caliente) == 2: print("* TE QUEMAS, el número oculto es MENOR... ¡te quedan N intentos!\n")
    else:
        if evaluar_diferencia(numero, numero_oculto, frio, caliente) == 0: print("* FRÍO, FRÍO, el número oculto es MAYOR... ¡te quedan N intentos!\n")
        elif evaluar_diferencia(numero, numero_oculto, frio, caliente) == 1: print("* CALIENTE, CALIENTE, el número oculto es MAYOR... ¡te quedan N intentos!\n")
        elif evaluar_diferencia(numero, numero_oculto, frio, caliente) == 2: print("* TE QUEMAS, el número oculto es MAYOR... ¡te quedan N intentos!\n")
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, frio: int, caliente: int):
    ...
    num = pedir_numero_usuario("¿Qué número es: ")
    while num != numero_oculto and intentos > 0:
        intentos -=1
        obtener_pista(num, numero_oculto, intentos, frio, caliente)
        num = pedir_numero_usuario("¿Qué número es: ")
    if num == numero_oculto:
        numero_adivinado = True
        intentos_realizados = total_intentos - intentos
    else: 
        numero_adivinado = False
        intentos_realizados = total_intentos
...
def configurar_juego() -> tuple:
    ...
    limpiar_pantalla()
    mostrar_titulo(4)
    valores_correcto = False
    while valores_correcto is False:
        try:
            minimo = int(input("Introduce el valor mínimo del rango: "))
            maximo = int(input("Introduce el valor máximo del rango: "))
            frio = int(input("Introduce el valor para la pista frío: "))
            caliente = int(input("Introduce el valor para la pista caliente: "))
            intentos = int(input("Introduce los intentos: "))
        except ValueError:
            mostrar_error("**ERROR** -> Introduce un valor correcto")
            valores_correcto = False
        finally:
            if minimo or maximo or frio or caliente or intentos is not None:
                valores_correcto = True
...
def comprobar_opcion(opcion):
    ...
    try:
        ...
        mostrar_error("**ERROR** -> Opción no valida")
...
def elegir_opcion_menu(msj: str) -> int:
    """
    ...
    opcion = comprobar_opcion(opcion)
...
def jugar(numero_oculto: int, intentos: int, frio: int, caliente: int):
    ...
    print("")
    if numero_adivinado:
        print(f"Lo has adivinado, te quedaban {intentos_realizados} intentos")
    else:
        print(f"Has fallado, te has quedado sin intentos. El número era {numero_oculto}")
    pausa(3)
...
def main():
    ...
    mostrar_configuracion(minimo, maximo, intentos, frio, caliente,); print("")
    salir = False

    while salir == False:
        limpiar_pantalla()
        mostrar_menu()
        opcion = elegir_opcion_menu("Elige una opción")
