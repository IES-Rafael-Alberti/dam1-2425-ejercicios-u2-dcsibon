...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    if numero_oculto < numero:
        pista += "MENOR... ¡te quedan {} intentos!\n".format(intentos)
    else:
        pista += "MAYOR... ¡te quedan {} intentos!\n".format(intentos)
...
def configurar_pistas(minimo: int, maximo: int) -> tuple:
    ...
    while not valido:
        try:
            frio = pedir_numero_usuario("Introduce la diferencia para FRÍO: ", minimo, maximo)
            caliente = pedir_numero_usuario("Introduce la diferencia para CALIENTE: ", 1, frio)
            if frio <= caliente:
...
def comprobar_opcion(opcion) -> bool:
	# Crear la documentación recomendada para esta función
    """Verifica si la nuestras opciones son válidas a los datos que ponemos.

    Esta función comprueba si el valor de 'opcion' se encuentra entre 1 a 4.
    Se utiliza para validar opciones en un menú o selección de opciones.
...
def jugar(numero_oculto: int, intentos: int, frio: int, caliente: int):
    ...
    numero_adivinado, intentos_realizados = adivina_el_numero(numero_oculto, intentos, 0, 100, frio, caliente)

    pausa()
    if numero_adivinado:
        print(f"\n¡Bravo! ¡Lo conseguiste en {intentos_realizados} intentos!")
    else:
        print(f"\n¡Lo siento! El número oculto era {numero_oculto}.")
    pausa(2)
...
def genera_numero_oculto(minimo: int, maximo: int) -> int:
	# Crear la documentación recomendada para esta función
    """
    Genera un número entero aleatorio dentro de el rango-

    Esta función devuelve un número entero aleatorio que se encuentra
    entre los valores "minimo" y "maximo".
    """
