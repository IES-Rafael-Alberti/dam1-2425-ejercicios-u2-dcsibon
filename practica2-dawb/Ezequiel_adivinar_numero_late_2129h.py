...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    while adivinando:
        ...
        while input_invalido:
            ...
            num = pedir_numero_usuario("¿Qué número es? ", minimo, maximo)
        ...
        if intentos_realizados == total_intentos:
            ...
            if num == numero_oculto:
                ...
                print(num, numero_oculto, intentos_realizados, frio, caliente)
                obtener_pista(num, numero_oculto, intentos_realizados, frio, caliente)
...
def configurar_rangos_numeros() -> tuple:
    ...
    while numero_invalido:
        try:
            minimo = pedir_numero_usuario("Número mínimo del rango posible para el número oculto: ")
            maximo = pedir_numero_usuario("Número máximo del rango posible para el número oculto: ")

            if type(minimo) == int and type(maximo) == int:
                diferencia = abs(minimo-maximo)
                if minimo > maximo:
                    raise ValueError("El valor mínimo no puede ser superior al máximo.")
                if diferencia < 100:
                    raise ValueError("El rango del número oculto debe se rigual o superior a 100.")
                else:
                    numero_invalido = False
            else:
                mostrar_error("No has introducido un número válido")
...
def configurar_pistas(minimo: int, maximo: int) -> tuple:
    ...
    while numero_invalido:
        ...
        try:
            frio = pedir_numero_usuario("Números de diferencia para la pista frío: ")
            caliente = pedir_numero_usuario("Números de diferencia para la pista caliente: ")

            if type(frio) == int and type(caliente) == int:
                ...
                if minimo < frio or frio > maximo:
                    numero_invalido = True
                    raise ValueError(f"La diferencia para la pista CALIENTE debe estar entre {minimo} y {maximo}!")
                else:
                    numero_invalido = False

                if minimo < caliente or caliente > maximo:
                    numero_invalido = True
                    raise ValueError(f"La diferencia para la pista CALIENTE debe estar entre {minimo} y {maximo}!")
                else:
                    numero_invalido = False
...
def configurar_intentos(rango_numero_oculto) -> int:
    ...
    while numero_invalido:
        try:
            intentos = pedir_numero_usuario("Intentos posibles máximos: ")
            if type(intentos) == int:
                execede_rango = rango_numero_oculto * 1.1
                if intentos <= 0:
                    raise ValueError("*ERROR* El número de intentos debe ser un número entero positivo!")
                if intentos > execede_rango:
...
def configurar_juego() -> tuple:
    ...
    diferencia = abs(minimo-maximo)
    intentos = configurar_intentos(diferencia)
...
def comprobar_opcion(opcion: int) -> bool:
    """
    Comprueba si la opción esta entre el rango correcto

        Args:
            opcion (int): La opción elegida por el usuario

        Returns:
            True => Si opcion esta entre el rango 1-4
            False => Si opcion no esta entre el rango 1-4
    """
...
def elegir_opcion_menu() -> int:
    ...
    while not opcion_correcta:
        ...
        mostrar_menu()
        opcion = pedir_numero_usuario("Elige => ")
...
def jugar(numero_oculto: int, minimo: int, maximo: int, intentos: int, frio: int, caliente: int):
    ...
    try:
        ...
        print(f"\n*UNEXPECTED ERROR* at jugar() function:\n{e}\n")
...
def genera_numero_oculto(minimo: int, maximo: int) -> int:
    ...
    Args:
        minimo (Int) => Número mínimo para el número oculto
        maximo (Int) => Número máximo para el número oculto
