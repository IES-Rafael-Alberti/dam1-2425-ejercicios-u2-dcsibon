...
def mostrar_titulo(seccion: int, intentos: int = 0):
    ...
    try:
        ...
        print("*ERRORS* La sección no es correcta...")
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if diferencia >= frio:
        return 0
    elif diferencia < frio and diferencia >= caliente:
        return 1
    else:
        return 2
...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    if evaluar_diferencia(numero, numero_oculto, frio, caliente) == 1:
        if numero_oculto < numero:
            ...
            print(f"* CALIENTE, CALIENTE, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n")
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    while numero_oculto != numero or intentos_restantes > 0:
    ...
    if intentos_restantes == 0:
        ...
        intentos_realizados = total_intentos - intentos_restantes
...
def configurar_rangos_numeros() -> tuple:
    ...
    while minimo:
        try:
            print("Introduce el máximo")
            maximo = pedir_numero_usuario()
            print("Introduce el mínimo")
            minimo = pedir_numero_usuario()

        except ValueError:
            raise ValueError("*ERROR* Algo ha ido mal...")
...
def configurar_intentos(rango_numero_oculto) -> int:
    ...
    intentos = False
    porcentaje = rango_numero_oculto * 0.10
    while intentos:
        try:
...
def configurar_juego() -> tuple:
    ...
    seccion = 4
    limpiar_pantalla()
    mostrar_titulo(seccion)
    
    minimo, maximo = configurar_rangos_numeros()
    frio, caliente = configurar_pistas()
    intentos = configurar_intentos(rango_numero_oculto = maximo - minimo)
