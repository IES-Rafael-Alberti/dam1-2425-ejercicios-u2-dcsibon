...
def mostrar_titulo(seccion: int, intentos: int = 0):
    ...
    try:
        ...
        mostrar_error(f"Ese índice de título no existe. {e}")
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if diferencia > frio:
        return 0
    elif frio > diferencia > caliente:
        return 1
    elif caliente > diferencia:
        return 2
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    while not finalizar:
        ...
        if numero == numero_oculto:
            return f"ENHORABUENA HAS ACERTADO! "
        elif numero != numero_oculto:
            obtener_pista()
        total_intentos -= 1
        print(f"Te quedan {total_intentos} intentos.")
...
def configurar_intentos(rango_numero_oculto) -> int:
    ...
    if intentos < 0:
        mostrar_error("*ERROR* El número de intentos debe ser un número entero positivo!")
    if intentos > rango_numero_oculto * 0.1:
        mostrar_error("*ERROR* El número de intentos no puede ser superior al 10% del rango del número oculto!")
...
def configurar_juego() -> tuple:
    ...
    minimo, maximo = genera_numero_oculto()
...
def comprobar_opcion(opcion):
    ...
    """
    Se asegura de que la opcion está entre los valores 1 y 4
    """
    return 1 <= opcion <= 4
...
def genera_numero_oculto(minimo, maximo):
	# Crear la documentación recomendada para esta función
    """
    Esta funcion genera un nº aleatorio entre el rango de los dos parametros proporcionados
    """
