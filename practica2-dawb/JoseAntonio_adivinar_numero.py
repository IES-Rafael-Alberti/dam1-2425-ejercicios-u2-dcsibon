...
def mostrar_titulo(seccion: int, intentos: int = 0):
    ...
    try:
        ...
        mostrar_error("No se encuentra en el rango establecido!!!")
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if numero > numero_oculto:
        ...
        if numero_oculto - numero >= frio:
            return frio
        elif numero - numero_oculto < caliente:
            return caliente
        elif numero_oculto - numero >= caliente:
            return caliente
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    intentos = 0
    while not assist:
        numero = pedir_numero_usuario(f"Introduce un numero del {minimo} al {maximo}: ")
        if intentos == total_intentos:
            return "No tienes más intentos!"
        elif numero > numero_oculto:
            evaluar_diferencia(numero,numero_oculto,frio,caliente)
            intentos += 1
            obtener_pista(numero_oculto,numero,intentos,frio,caliente)
        elif numero < numero_oculto:
            evaluar_diferencia(numero, numero_oculto, frio, caliente)
            intentos += 1
        else:
            ...
            return (f"Lo has resuelto, el numero era: {numero_adivinado} y lo acabaste en {intentos_realizados} intentos.")
...
def configurar_rangos_numeros() -> tuple:
    ...
    try:
        minimo = pedir_numero_usuario("Introduce un nuevo minimo: ",None,None)
        maximo = pedir_numero_usuario("Introduce un nuevo maximo: ",minimo,None)
...
def jugar(numero_oculto: int, intentos: int, frio: int, caliente: int):
    ...
    numero_adivinado, intentos_realizados = adivina_el_numero(numero_oculto,total_intentos,minimo,maximo,frio,caliente)
...
def main():
    ...
    mostrar_configuracion(minimo,maximo,caliente,intentos,frio)
