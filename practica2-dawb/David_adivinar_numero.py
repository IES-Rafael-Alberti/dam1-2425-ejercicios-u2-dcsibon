...
def pausa(tiempo = 0, tecla_enter = False, limpiar = True):
    ...
    if tecla_enter is True:
        input("Presiona ENTER para continuar...")
    elif tiempo > 0:
        time.sleep(tiempo)
    if limpiar_pantalla is True:
        limpiar_pantalla()
...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    if numero_diferencia == 0:
        ...
        if numero > numero_oculto:
            pista += f"el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
        else:
            pista += f"el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    while total_intentos > 0:
        numero_jugador = pedir_numero_usuario("¿Qué número es?", minimo, maximo)
        intentos_realizados += 1
        if numero_jugador == numero_oculto:
            numero_adivinado = True
            return numero_adivinado, intentos_realizados
        else:
            obtener_pista(numero_jugador, numero_oculto, total_intentos, frio, caliente)
        return numero_adivinado, intentos_realizados
...
def configurar_rangos_numeros() -> tuple:
    ...
    minimo = pedir_numero_usuario("Ingresa el número minimo: ")
    maximo = pedir_numero_usuario("Ingresa el número máximo: ")
...
def configurar_pistas(minimo: int, maximo: int) -> tuple:
    ...
    try:
        frio = pedir_numero_usuario("Introduce la diferencia FRIO: ")
        caliente = pedir_numero_usuario("Introduce la diferencia CALIENTE: ")
...
def genera_numero_oculto(minimo, maximo):
    """
    Genera un número al azar desde el parámetro 1 hasta el parámetro 2 (minimo y máximo)

    Args:
        minimo (int): Número mínimo para generar el número al azar.
        maximo (int): Número máximo para generar el número al azar.

    Returns:
        int: El número generado al azar
    """
	# Crear la documentación recomendada para esta función
