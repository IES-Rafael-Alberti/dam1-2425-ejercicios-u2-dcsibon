...
def pausa(tiempo = 0, tecla_enter = False, limpiar = True):
    ...
    if tecla_enter and tiempo == 0:
        valor = input("\nPresione ENTER para continuar...")
        
    if tecla_enter and limpiar:
        limpiar_pantalla()
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if abs(numero_oculto - numero) > abs(numero_oculto - frio):
        valor_referencia = 0
        
    elif abs(numero_oculto - frio) > abs(numero_oculto - numero) < abs(numero_oculto - caliente):
        valor_referencia = 1
...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    diferencia = evaluar_diferencia()
    
    if diferencia == 0 and numero > numero_oculto:
        frase = f"* FRÍO, FRÍO, el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
    
    elif diferencia == 0 and numero < numero_oculto:
        frase = f"* FRÍO, FRÍO, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
    
    elif diferencia == 1 and numero > numero_oculto:
        frase = f"* CALIENTE, CALIENTE, el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
        
    elif diferencia == 1 and numero < numero_oculto:
        frase = f"* CALIENTE, CALIENTE, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
        
    elif diferencia == 2 and numero > numero_oculto:
        frase = f"* TE QUEMAS, el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
        
    elif diferencia == 2 and numero < numero_oculto:
        frase = f"* TE QUEMAS, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
...
def configurar_pistas(minimo: int, maximo: int) -> tuple:
    ...
    try:
        while bandera:
            frio = pedir_numero_usuario()
            caliente = pedir_numero_usuario() 
            
            #Valores iguales o a la diferencia(resta coño)
            if frio <= caliente:
...
def configurar_intentos(rango_numero_oculto) -> int:
    ...
    try:
        ...
        diez_porciento = (rango_numero_oculto * 10) / 100
        
        if intentos < 0:
            raise ValueError("*ERROR* El número de intentos debe ser un número entero positivo!")
        if intentos < diez_porciento:
            raise ValueError(f"*ERROR* El número de intentos no puede ser superior al 10% del rango del número oculto!")
...
def configurar_juego(rango_numero_oculto) -> tuple:
    """
...
def elegir_opcion_menu() -> int:
    ...
    try: 
        while bandera:
            ...
            if comprobar_opcion(int(opcion)):
