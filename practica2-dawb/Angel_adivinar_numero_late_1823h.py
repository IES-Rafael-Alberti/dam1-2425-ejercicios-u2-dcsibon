...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    if evaluar_diferencia (40, 80, 15, 5): 
        0 
    if evaluar_diferencia (40, 45, 15, 5): 
        1 
    if evaluar_diferencia (48, 50, 15, 5): 
        2 
...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    if evaluar_diferencia == 0:
        pista = "MAYOR" if intentos < numero_oculto else "MENOR"
        print(f"**FRIO FRIO** El número es {pista}... ¡te quedan [{N - 1} intentos!]")
    elif evaluar_diferencia == 1:
        pista = "mayor" if intentos < numero_oculto else "menor"
        print(f"**CALIENTE CALIENTE** El número es {pista}... ¡te quedan [{N - 1} intentos!]")
    elif evaluar_diferencia == 2:
        pista = "mayor" if intentos < numero_oculto else "menor"
        print(f"**TE QUEMAS** El número es {pista}...¡te quedan [{N - 1} intentos!]]")
...
def configurar_intentos(rango_numero_oculto) -> int:
    ...
    intentos = int(input("Introduzca el numero de intentos: "))
