...
TITULOS = (
    ...
    "--- ADIVINA EL NÚMERO OCULTO EN {intentos_tit} INTENTOS ---",
...
def mostrar_titulo(seccion: int, intentos: int = 0):
    ...
    try:
        if intentos > 0:
            print(TITULOS[seccion].format(intentos_tit = intentos) + "\n\n")
        else:
...
def evaluar_diferencia(numero: int, numero_oculto: int, frio: int, caliente: int) -> int:
    ...
    distancia = abs(numero_oculto - numero)

    if distancia > frio:
        return 0
    
    if distancia <= frio and distancia >= caliente:
        return 1
...
def obtener_pista(numero: int, numero_oculto: int, intentos: int, frio: int, caliente: int) -> str:
    ...
    es_mayor = numero_oculto > numero


    if diferencia == 0:
        if es_mayor:
            pista = f"* FRÍO, FRÍO, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
        else:
            pista = f"* FRÍO, FRÍO, el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
    elif diferencia == 1:
        if es_mayor:
            pista = f"* CALIENTE, CALIENTE, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
        else:
            pista = f"* CALIENTE, CALIENTE, el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
    else:
        if es_mayor:
            pista = f"* TE QUEMAS, el número oculto es MAYOR... ¡te quedan {intentos} intentos!\n"
        else:
            pista = f"* TE QUEMAS, el número oculto es MENOR... ¡te quedan {intentos} intentos!\n"
...
def adivina_el_numero(numero_oculto: int, total_intentos: int, minimo: int, maximo: int, frio: int, caliente: int):
    ...
    while not numero_adivinado and intentos_realizados < total_intentos:
        ...
        if numero != None:
            ...
            elif total_intentos > intentos_realizados:
...
def configurar_rangos_numeros() -> tuple:
    ...
    while not rangos_validos:

        try:
            minimo = pedir_numero_usuario("Dame el mínimo: ")
            maximo = pedir_numero_usuario("Dame el máximo: ")

            if minimo >= maximo:
                raise ValueError("El valor mínimo no puede ser superior al máximo.")
            elif abs(maximo-minimo) < 100:
...
def comprobar_opcion(opcion: int) -> bool:
    ...
    """
    Compruebaque la opción del menú está entre 1 y 4.

    Args:
        opcion (int): opción de menú seleccionada por el usuario.

    Returns:
        (bool): True si es correcto y False sino.
    """    
    return 1 <= opcion <= 4
