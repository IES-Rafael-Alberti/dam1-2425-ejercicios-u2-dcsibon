...
def calcular_operacion(num1: float, num2: float, operador: str) -> float:
    ...
    resultado = "0.0"
    operador = pedir_entrada()
    if operador in ['+', '-', '*', 'x', '/', ':', '**', 'exp']:
                    operador = OPERADORES
    else:
        # Convertir la entrada a número usando float para realizar las operaciones
        num1 = float(pedir_entrada)
        num1 = float(pedir_entrada)

        if resultado == "0.0":
                        calculo_actual = num1  # Primer número ingresado
        else:
            ...
            if operador == '+':
                ...
                calculo_actual = str(float(calculo_actual) ** num1, num2)
...
def realizar_calculo():
    ...
    while realizando_calculos:
        ...
        if entrada == "cancelar":
            print("Secuencia de calculo cancelada.")
            break
