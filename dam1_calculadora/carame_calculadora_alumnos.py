...
def mostrar_error(indice_error: int, msj_error = None):
    ...
    for indice_error in MENSAJES_ERROR:
        if indice_error == 1:
            print("Problemas al intentar limpiar la pantalla {error}")
        elif indice_error == 2:
            print("Error al configurar los decimales. Formato: decimales <n>.")
        elif indice_error == 3:
            print("Entrada no válida. Ingrese número, operador, 'resultado', 'cancelar' o <ENTER> para finalizar el cálculo.")
        elif indice_error == 4:
            print("Error: Introduzca un operador antes de otro número.")
        elif indice_error == 5:
            print("Comando no reconocido. Escriba 'lista' para ver las operaciones disponibles.")
        elif indice_error == 6:
            print("Error: no es posible la división por 0! Introduzca otro valor diferente a 0...")
        elif indice_error == 7:
...
def multiplicar(num1 : float,num2 : float)->int :
    ...
    round(num1)
    round(num2)
    n1 = num1 #Guardo los valores originales de num1
    n2 = num2 #Guardo los valores originales de num2
    if num2 < 0: #Quito los negativos de num2 si hay
        num2 = str(num2)
        num2 = (num2[1])
        num2 = float(num2)
    if num1 < 0: #Quito los negativos de num1 si hay
        num1 = str(num1)
        num1 = (num1[1])
        num1 = float(num1)
    resultado = 0
    ...
    if not es_resultado_negativo(n1,n2): #Saco si el resultado es negativo o positivo gracias a los valores antes recibidos
        return int(resultado)
    else:
        return int(-resultado)

#Esto es solo para el commit
...
def potencia(num1:float,num2:float):
    """
    Realiza la potencia de un número elevado a otro.
    Args:
        num1 (float): Primer número.
        num2 (float): Segundo número.
    Returns:
    ...
    round(num2)
    resultado = 1
    for i in range(0,int(num2)):#Bucle for para hacer la potencia
        resultado = multiplicar(resultado,num1)#Llama a la funcion multiplicar
    if not es_resultado_negativo(num1,num2):#Llama a la funcion es_resultado_negativo
        return int(resultado)
    else:
        return int(-resultado)
