...
def limpiar_pantalla():
    ...
    try:
         ...
         if os.name != "posix" and os.name != "nt" :
            raise ValueError (MENSAJES_ERROR(1))
...
def pausa(tiempo = 0, tecla_enter = False):
    """
...
def sumar(num1 = float, num2 = float) -> float:
 ...
 """
Recibe dos numeros en float que son inputs de otra función  y retorna el resultado de la operación suma.
...
def es_resultado_negativo(num1: float, num2: float) -> bool:
    ...
    Si el resultado es de una multiplicación o división en la que los signos sean iguales 
    se concatena el signo positivo, si no se concatena el signo negativo.
    """
...
def multiplicar(num1: float, num2: float  ) -> int :
    ...
    numero_1 = round(num1)
    numero_2 = round(num2)
    for i in range(1,numero_2 + 1): #de esta manera estamos sumando num1 las veces que num2 pide que en si es el significado de la mmultiplicación.
        multiplicacion += numero_1
    
    return print(es_resultado_negativo() + multiplicacion) 
...
def dividir(num1: float, num2: float) -> int:
    ...
    Note:
        Debe redondear los números recibidos a enteros para trabajar.        
    """
    division_correcta =None
    while not division_correcta:
        try:

            cociente = 0
            dividendo = round(num1)
            divisor = round(num2) 
            division= 0 
            while dividendo != 0:
            ...
            return print(es_resultado_negativo()+cociente)
...
def potencia(num: float,exp:float ) -> int:
    # TODO: 
    ...
    num_elevado = 0
    if exponente != 0 :
        for i in range (1, exponente + 1) :#la potencia es la suma de las muliplicaciones 
            num_elevado += multiplicar(numero)
    else:
        num_elevado = 1 
...
def pedir_entrada(msj: str) -> str:
    ...
    msj= input("ingrese número para operar: ").strip()
    mensaje = msj
...
def calcular_operacion(num1: float, num2: float, operador: str) -> float:
    ...
    suma= OPERADORES(1)
    resta=OPERADORES(2)
    multiplica= OPERADORES(3)
    dividi= OPERADORES(4)
    potenci = OPERADORES(5)
    ...
    elif operado ==multiplica :
       resultado = multiplicar(numero1,numero2)
    elif operado == dividi:
        resultado = dividir(numero1,numero2)
    elif operado == potenci:
...
def obtener_operaciones() -> str:
    ...
    while True:
        realizar_calculo(resultado, decimales)
        comando = input().strip()
        ...
        if comando == "lista":
            ...
            print("        cancelar => volver sin actualizar resultado de la calculadora")
