...
def pausa(tecla_enter = False, tiempo = 0):
    """
...
def es_resultado_negativo(num1: float, num2: float) -> bool:
    ...
    Returns:
        es_negativo (bool): Dependiendo de si el resultado es negativo o no.
    """
...
def multiplicar(num1: float, num2: float) -> int:
    ...
    # Redondeamos los números a 0 decimales, como si fueran entero.
    num1 = round(num1, 0)
    num2 = round(num2, 0)
    resultado = 0
    
    # Después de hacer compatibles todos los números según lo pedido, hacemos un for donde por cada vez que i obtiene 
    # un valor, se suma el número.
    for i in range(int(abs(num2))):
        resultado += int(abs(num1))

    # Aquí comprobamos si el resultado es negativo o no con la función que se encarga de ello.
...
def dividir(num1: float, num2: float) -> int:
    ...
    contador = int(abs(num1))
    # Si el divisor es 0, muestra el error 5.
    if num2 == 0:
        raise ZeroDivisionError(mostrar_error(5))
    # Este while va controlando que el dividendo sea mayor que el divisor y así restando el número tantas veces como haga falta.
    else:
        while contador >= abs(int(num2)):
            contador -= abs(int(num1))
            resultado += 1
        # Aquí comprobamos si el resultado es negativo o no con la función que se encarga de ello.
        if es_resultado_negativo(num1, num2):
            resultado = str(resultado)
...
def potencia():
    """
    Esta función recoge dos números y hace que el segundo sea la potencia del primero.
    ...
    # Aquí comprobamos si el exponente es 0 o negativo porque el resultado será siempre 1 o 0 dependiendo el caso.
    if num2 == 0:
        resultado = 1
    elif num2 < 0:
        resultado = 0
...
def obtener_operaciones() -> str:
    ...
    # Ya no sé ni lo que hago Diego.
    return lista


def realizar_calculo(decimales: int, resultado_almacenado: int) -> float:
    """
...
def main():
    ...
    while not desea_salir:
        ...
        if entrada == "":
            ...
            resultado = round(resultado, int(decimales))
