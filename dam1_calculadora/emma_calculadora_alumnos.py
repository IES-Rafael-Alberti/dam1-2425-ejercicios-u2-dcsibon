...
MENSAJES_ERROR = (
    f"Problemas al intentar limpiar la pantalla {error}",
    f"Error al configurar los decimales. Formato: decimales <n>.",
    f"Entrada no válida. Ingrese número, operador, 'resultado', 'cancelar' o <ENTER> para finalizar el cálculo.",
    f"Error: Introduzca un operador antes de otro número.",
    f"Comando no reconocido. Escriba 'lista' para ver las operaciones disponibles.",
    f"Error: no es posible la división por 0! Introduzca otro valor diferente a 0...",
    f"Se produjo un error: {error}"
)
...
def mostrar_error(indice_error: int, msj_error = None):
    ...
    try:
        if msj_error:
            mensaje = mensaje.format(error = msj_error)
            print(f"**ERR0R** {mensaje}")
...
def realizar_calculo(resultado_almacenado: int, decimales: float):
    """
...
def desea_salir() -> bool:
    ...
    return input("¿Desea salir de la calculadora? (s/n) ").strip().lower() != 'n'
...
def main():
    ...
    while not desea_salir():
        ...
        if entrada == "":
            ...
            resultado = 0.0
            print(f"Resultado reiniciado a {resultado}.")
            ...
            resultado_calculo = realizar_calculo(decimales, resultado)
